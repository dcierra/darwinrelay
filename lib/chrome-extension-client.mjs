...
function normalizeBackgroundChromeMethod(method) {
  ...
  // path: the managed DarwinRelay workspace. Raw tabs.open used to create loose tabs.
  return method === "tabs.open" ? "workspace.open" : method;
}

export function backgroundChromeSocketPath(dataDir = null) {
  const root = dataDir || process.env.DARWINRELAY_DATA_DIR
    || path.join(os.homedir(), "Library", "Application Support", "DarwinRelay");
  return process.env.DARWINRELAY_CHROME_SOCKET || path.join(root, "chrome-background.sock");
}
