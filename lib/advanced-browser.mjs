...
export function advancedBrowserConfig() {
  const name = String(process.env.MAC_DEV_BRIDGE_ADVANCED_BROWSER_NAME || "default");
  if (!/^[A-Za-z0-9_-]{1,64}$/.test(name)) {
    throw new Error("MAC_DEV_BRIDGE_ADVANCED_BROWSER_NAME must match [A-Za-z0-9_-]{1,64}");
  }
  const runtimeDir = process.env.MAC_DEV_BRIDGE_ADVANCED_BROWSER_RUNTIME_DIR
    ? path.resolve(process.env.MAC_DEV_BRIDGE_ADVANCED_BROWSER_RUNTIME_DIR.replace(/^~(?=$|\/)/, os.homedir()))
    : path.join(os.homedir(), ".config", "browser-harness", "runtime");
  const socketPath = process.env.MAC_DEV_BRIDGE_ADVANCED_BROWSER_SOCKET
    ? path.resolve(process.env.MAC_DEV_BRIDGE_ADVANCED_BROWSER_SOCKET.replace(/^~(?=$|\/)/, os.homedir()))
    : path.join(runtimeDir, `bu-${name}.sock`);
  return {
    enabled: envTrue(process.env.MAC_DEV_BRIDGE_ADVANCED_BROWSER),
    name,
    runtimeDir,
    socketPath,
  };
...
export async function advancedBrowserRequest(request, {
  ...
  if (!config.enabled) {
    const error = new Error("Advanced Browser/CDP backend is disabled. Set MAC_DEV_BRIDGE_ADVANCED_BROWSER=1 before starting MDB to opt in.");
