...
- There are **no per-conversation capability leases yet**. `mcp-http.mjs` now propagates non-secret request/session provenance into the shared `bridge.mjs` child, so audit records can distinguish HTTP requests and, when available, an MCP-session or OAuth-grant lineage. That is observability only: an OAuth grant can span multiple chats, clients may omit `Mcp-Session-Id`, and correlation metadata does not revoke or constrain authority. What is enforceable remains process-global until the execution-lease work lands. **Every credential that reaches `/mcp` already has unrestricted shell.**
...
Request provenance is deliberately separate from credentials. `bridge.mjs` generates a fresh opaque `correlationId` for every request and keeps it in `AsyncLocalStorage` so concurrent calls cannot cross-wire provenance. The HTTP front end adds its own opaque request id. If a client sends `Mcp-Session-Id`, DarwinRelay never writes that raw value to the audit log: `mcp-http.mjs` turns it into a process-local HMAC identifier first. OAuth access/refresh records instead carry a separately generated opaque session id that survives refresh and restart; it is not a hash of either token. Audit/session correlation is **not** an authorization boundary and must not be interpreted as proof that one identifier equals one ChatGPT conversation.
