...
- Added a deterministic AppKit integration fixture and real native end-to-end test covering semantic mutation/postconditions, ScreenCaptureKit window capture, Vision OCR, window movement/resize, AX dialogs, concurrent visual waits, slider drag/drop, and real open/save panels. CI always builds the fixture and skips the permission-dependent runtime section when TCC is unavailable; hosted runners validate drag construction/routing even when their desktop session refuses CGEvent pointer delivery, while interactive Macs additionally assert the slider state change.
