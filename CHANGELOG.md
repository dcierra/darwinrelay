...
- Added a background-first desktop GUI guard for native apps such as Slack. Model-driven AppleScript/JXA app control and foreground `open` calls are blocked by default; the old model-controlled env bypass is gone. Genuine foreground UI work requires a single-use, app-scoped, maximum-five-minute grant from `scripts/approve-foreground-gui.sh`.
